...
class ScenarioInfo:
    ...
    def binary(self):
        ...
        buffer.Write(self.MapName.encode(ed6sc.CODE_PAGE).ljust(0xA, b'\x00')[:0xA])
        buffer.Write(self.Location.encode(ed6sc.CODE_PAGE).ljust(0xE, b'\x00')[:0xE])
    ...
    def open(self, scenafile):
        ...
        self.MapName            = fs.read(0xA).decode(ed6sc.CODE_PAGE).split('\x00', 1)[0]
        self.Location           = fs.read(0xE).decode(ed6sc.CODE_PAGE).split('\x00', 1)[0]
    ...
    def DiasmInstructionCallback(self, data):
        ...
        if inst.OpCode == ed6sc.SetPlaceName:
        ...
        if ExtractText:
            if inst.OpCode == ed6sc.ChrTalk:
                text = inst.Operand[1]
                textPosTable[self.scenaName].append([s.dump() for s in text])

            elif inst.OpCode == ed6sc.AnonymousTalk:
                text = inst.Operand[0]
                textPosTable[self.scenaName].append([s.dump() for s in text])

            elif inst.OpCode == ed6sc.NpcTalk:
                for text in inst.Operand[1:]:
                    textPosTable[self.scenaName].append([s.dump() for s in text])

            elif inst.OpCode in [ed6sc.Menu, ed6sc.SetChrName]:
                text = inst.Operand[-1]
                textPosTable[self.scenaName].append([s.dump() for s in text])
    ...
    def loadScpStringList(self, paramList):
        return [ed6sc.ScpString(**p) for p in paramList]

    def DisassembleBlocks(self, fs):
        disasm = Disassembler(ed6sc.ed6sc_op_table, self.DiasmInstructionCallback)
    ...
    def FormatInstructionCallback(self, data, text):
        if data.Instruction.OpCode == ed6sc.SetPlaceName and self.PlaceName:
            return [text[0] + ' # ' + self.PlaceName]

    def FormatCodeBlocks(self):
        ed6sc.ed6sc_op_table.FunctionLabelList = self.GenerateFunctionLabelList(self.CodeBlocks)
        disasm = Disassembler(ed6sc.ed6sc_op_table, self.FormatInstructionCallback)
    ...
    def GenerateHeader(self, filename):
        ...
        hdr = []
        hdr.append('from ED6SCScenarioHelper import *')
        hdr.append('')
        hdr.append('SetCodePage("%s")'                          % ed6sc.CODE_PAGE)
        hdr.append('')
...
def main():
    ...
    if use_index_lable:
        ...
        ed6sc.USE_INDEX_LABEL_NAME = True
        ed6sc.UseIndexLabelName()
