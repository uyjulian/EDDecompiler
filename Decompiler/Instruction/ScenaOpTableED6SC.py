...
ed6sc_op_table = InstructionTable(GetOpCode, WriteOpCode, DefaultGetLabelName, CODE_PAGE)
...
class ScpExpression:
    ...
    def __str__(self):
        ...
        asm = Assembler2.Disassembler(ed6sc_op_table)

        txt = 'scpexpr(%s' % ExpressionOperantions[self.Operation]
        for inst in self.Operand:
            data = HandlerData(HANDLER_REASON_FORMAT)
            data.Instruction    = inst
            data.TableEntry     = ed6sc_op_table[inst.OpCode]
...
for op in ed6sc_op_list:
    ed6sc_op_table[op.OpCode] = op
    op.Container = ed6sc_op_table
...
if __name__ == '__main__':
    valid = 0
    for inst in ed6sc_op_list:
        if inst.OpName[:3] != 'OP_':
            valid += 1
    print('known: %d (%d%%)' % (valid, valid / len(ed6sc_op_list) * 100))
    print('total: %d' % len(ed6sc_op_list))
    console.pause()
